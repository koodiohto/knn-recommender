export default class KNNRecommender {
    ...
    constructor(userItemMatrix) {
        this.userToUserSimilarityMap = {};
        this.userToRowNumberMap = {};
    ...
    addNewEmptyUserToDataset(userId) {
        let userArray = new Array(this.userItemMatrix[0].length);
        userArray[0] = userId;
 ...
 */
            ...
            for (let j = 1; j < userRecommendations.length; j++) {
                if ((!onlyUnique || !recommendationsAlreadyIncluded[j]) &&
                    ...
                    newRecommendations[newRecommendationCounter] = {
                        itemId: this.userItemMatrix[0][j],
     ...
     */
    calculateDistancesInZeroOneUserItemMatrixAndCreateUserToRowAndItemToColumnMapInChunks() {
        return new Promise((resolve, reject) => {
            ...
            for (let i2 = 1; i2 < rows; i2++) {
                ...
                for (let j = 1; j < columns; j++) { //first column contains the user name, start with second
                    if (!itemIdToColumnNumberMapInitiated) {
                        this.itemIdToColumnNumberMap[this.userItemMatrix[0][j]] = j;
                    }
                ...
                userToOtherUsersSimilarityList[userToOtherUsersCounter] = { otherUserId: this.userItemMatrix[i2][0], similarity: jaccardSimilarity };
    ...
    sortUserToOtherUsersSimilarityListByUserToUserSimilarityDescending(userToOtherUsersSimilarityList) {
