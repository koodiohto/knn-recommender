...
initializeRecommenderForUserId | ```userId: string ``` | void | (Re)-initialize the recommender only for one userId. This is significanly faster than initializing the recommender for all users, so you should use this method when possible. | ```kNNRecommender.initializeRecommenderForUserId('user 1')```
